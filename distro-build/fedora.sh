...
write_plugin() {
	...
	TARBALL_URL['aarch64']="${ROOTFS_FILESERVER_URL}/fedora-aarch64-pd-${CURRENT_VERSION}.tar.xz"
	TARBALL_SHA256['aarch64']="$(sha256sum "${ROOTFS_DIR}/fedora-aarch64-pd-${CURRENT_VERSION}.tar.xz" | awk '{ print $1}')"
	TARBALL_URL['x86_64']="${ROOTFS_FILESERVER_URL}/fedora-x86_64-pd-${CURRENT_VERSION}.tar.xz"
