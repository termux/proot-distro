...
write_plugin() {
	...
	TARBALL_URL['aarch64']="${ROOTFS_FILESERVER_URL}/pardus-aarch64-pd-${CURRENT_VERSION}.tar.xz"
	TARBALL_SHA256['aarch64']="$(sha256sum "${ROOTFS_DIR}/pardus-aarch64-pd-${CURRENT_VERSION}.tar.xz" | awk '{ print $1}')"
	TARBALL_URL['i686']="${ROOTFS_FILESERVER_URL}/pardus-i686-pd-${CURRENT_VERSION}.tar.xz"
	TARBALL_SHA256['i686']="$(sha256sum "${ROOTFS_DIR}/pardus-i686-pd-${CURRENT_VERSION}.tar.xz" | awk '{ print $1}')"
	TARBALL_URL['x86_64']="${ROOTFS_FILESERVER_URL}/pardus-x86_64-pd-${CURRENT_VERSION}.tar.xz"
