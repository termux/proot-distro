...
_HELP_COMMANDS = {
    ...
    "install": lambda: (
        msg(
          "\n"
          f"{C['BYELLOW']}Usage: {C['BCYAN']}{PROGRAM_NAME} {C['GREEN']}install {C['CYAN']}[OPTIONS] DISTRIBUTION{C['RST']}\n"
          "\n"
          f"{C['CYAN']}Command aliases: {C['GREEN']}add{C['CYAN']}, {C['GREEN']}i{C['CYAN']}, {C['GREEN']}in{C['CYAN']}, {C['GREEN']}ins{C['RST']}\n"
          "\n"
          f"{C['CYAN']}Install a specified Linux distribution.{C['RST']}\n"
          "\n"
          f"{C['CYAN']}Options:{C['RST']}\n"
          "\n"
          f"  {C['GREEN']}--help                        {C['CYAN']}- Show this help information.{C['RST']}\n"
          "\n"
          f"  {C['GREEN']}--name [new-name]             {C['CYAN']}- Set a custom alias for installed{C['RST']}\n"
          f"                                  {C['CYAN']}distribution.{C['RST']}\n"
          "\n"
          f"  {C['GREEN']}--architecture [arch]        {C['CYAN']}- Override the architecture to install.{C['RST']}\n"
          f"                                  {C['CYAN']}Valid values: aarch64, arm, i686,{C['RST']}\n"
          f"                                  {C['CYAN']}riscv64, x86_64.{C['RST']}\n"
          "\n"
          f"  {C['GREEN']}--url [url]                  {C['CYAN']}- Override the rootfs download URL.{C['RST']}\n"
          f"                                  {C['CYAN']}When specified, --checksum is not{C['RST']}\n"
          f"                                  {C['CYAN']}required but recommended.{C['RST']}\n"
          "\n"
          f"  {C['GREEN']}--checksum [sha256]          {C['CYAN']}- Override the rootfs SHA-256 checksum.{C['RST']}\n"
          f"                                  {C['CYAN']}When --url is not given, overrides only{C['RST']}\n"
          f"                                  {C['CYAN']}the checksum from the config.{C['RST']}\n"
          "\n"
          f"  {C['GREEN']}--strip-path-components [n]  {C['CYAN']}- Strip N leading path components when{C['RST']}\n"
          f"                                  {C['CYAN']}extracting the rootfs archive{C['RST']}\n"
          f"                                  {C['CYAN']}(like tar --strip-components).{C['RST']}\n"
          f"                                  {C['CYAN']}Overrides the value from the config.{C['RST']}\n"
        ),
        show_version(),
        msg(),
    ),
...
def command_help() -> None:
    msg(
      ...
      "\n"
      f"{C['CYAN']}Hint: type command '{C['GREEN']}{PROGRAM_NAME} list{C['CYAN']}' to get a list of the{C['RST']}\n"
      f"{C['CYAN']}supported distributions. Pick a distro alias and run the next{C['RST']}\n"
      f"{C['CYAN']}command to install it: {C['GREEN']}{PROGRAM_NAME} install <alias>{C['RST']}\n"
      "\n"
