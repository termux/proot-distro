...
def make_writable(dir_fd: int) -> None:
    ...
    try:
        st = os.fstat(dir_fd)
        os.fchmod(dir_fd, stat.S_IMODE(st.st_mode) | stat.S_IRWXU)
    except OSError:
        pass
...
def copy_file_at(src_dir_fd: int, src_name: str,
    ...
    Both ends are opened with O_NOFOLLOW, so a symlink planted at either
    name is refused rather than followed.
    """
    sfd = open_file_at(src_dir_fd, src_name, os.O_RDONLY)
    try:
        if src_st is None:
            src_st = os.fstat(sfd)
        dfd = open_file_at(
...
def rmtree_at(dir_fd: int, name: str, *, force: bool = False) -> None:
    ...
    try:
        ...
        if not force:
            raise
        # Cannot descend: make the entry itself readable from here.
        try:
            os.chmod(name, stat.S_IMODE(st.st_mode) | stat.S_IRWXU,
                     dir_fd=dir_fd, follow_symlinks=True)
        except OSError:
            pass
        fd = opendir_at(dir_fd, name)
